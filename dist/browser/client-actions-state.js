...
export async function browserCookies(baseUrl, opts = {}) {
    ...
    return await fetchBrowserJson(`${baseUrl}/cookies${suffix}`, { timeoutMs: 20000 });
}
...
export async function browserStorageGet(baseUrl, opts) {
    ...
    if (opts.profile)
        q.set("profile", opts.profile);
    const suffix = q.toString() ? `?${q.toString()}` : "";
    return await fetchBrowserJson(`${baseUrl}/storage/${opts.kind}${suffix}`, { timeoutMs: 20000 });
}
...
export async function browserStorageClear(baseUrl, opts) {
    const q = buildProfileQuery(opts.profile);
    return await fetchBrowserJson(`${baseUrl}/storage/${opts.kind}/clear${q}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ targetId: opts.targetId }),
        timeoutMs: 20000,
