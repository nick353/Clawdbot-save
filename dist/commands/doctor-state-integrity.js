...
export async function noteStateIntegrity(cfg, prompter, configPath) {
    ...
    if (!stateDirExists) {
        warnings.push(`- CRITICAL: state directory missing (${stateDir}). Sessions, credentials, logs, and config are stored there.`);
    ...
    if (stateDirExists && process.platform !== "win32") {
        try {
            ...
            if ((stat.mode & 0o077) !== 0) {
                warnings.push(`- State directory permissions are too open (${stateDir}). Recommend chmod 700.`);
    ...
    if (configPath && existsFile(configPath) && process.platform !== "win32") {
        try {
            ...
            if ((stat.mode & 0o077) !== 0) {
                warnings.push(`- Config file is group/world readable (${configPath}). Recommend chmod 600.`);
                const tighten = await prompter.confirmSkipInNonInteractive({
    ...
    if (entries.length > 0) {
        ...
        if (mainEntry?.sessionId) {
            ...
            if (!existsFile(transcriptPath)) {
                warnings.push(`- Main session transcript missing (${transcriptPath}). History will appear to reset.`);
            }
            else {
