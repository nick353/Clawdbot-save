...
async function collectChannelStatus(params) {
    ...
    const catalogEntries = listChannelPluginCatalogEntries().filter((entry) => !installedIds.has(entry.id));
...
export async function setupChannels(cfg, runtime, prompter, options) {
    ...
    const getChannelEntries = () => {
        ...
        const catalog = listChannelPluginCatalogEntries().filter((entry) => !installedIds.has(entry.id));
