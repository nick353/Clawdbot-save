...
import { addAllowlistEntry, analyzeShellCommand, evaluateExecAllowlist, maxAsk, minSecurity, requiresExecApproval, resolveSafeBins, recordAllowlistUse, resolveExecApprovals, resolveExecApprovalsFromFile, } from "../infra/exec-approvals.js";
...
async function runExecProcess(opts) {
    ...
    let stdin;
    if (opts.sandbox) {
        child = spawn("docker", buildDockerExecArgs({
...
export function createExecTool(defaults) {
    ...
    return {
        ...
        execute: async (_toolCallId, args, signal, onUpdate) => {
            ...
            if (host === "node") {
                const approvals = resolveExecApprovals(agentId, host === "node" ? { security: "allowlist" } : undefined);
            ...
            if (host === "gateway" && !bypassApprovals) {
                ...
                const allowlistSatisfied = hostSecurity === "allowlist" && analysis.ok ? allowlistEval.allowlistSatisfied : false;
                const requiresAsk = requiresExecApproval({
                    ask: hostAsk,
                    security: hostSecurity,
                    analysisOk: analysis.ok,
                ...
                if (allowlistMatches.length > 0) {
                    ...
                    for (const match of allowlistMatches) {
                        if (seen.has(match.pattern))
                            continue;
                        seen.add(match.pattern);
                        recordAllowlistUse(approvals.file, agentId, match, params.command, analysis.segments[0]?.resolution?.resolvedPath);
                    }
                }
            }
            const effectiveTimeout = typeof params.timeout === "number" ? params.timeout : defaultTimeoutSec;
