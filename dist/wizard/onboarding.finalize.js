...
export async function finalizeOnboardingWizard(options) {
    ...
    if (!opts.skipUi && gatewayProbe.ok) {
        if (hasBootstrap) {
            ...
            if (!browserSupport.ok) {
                ...
                await prompter.note("Opening Control UI automatically after onboarding (no extra prompts).", "Open Control UI");
            }
        }
    }
    else if (opts.skipUi) {
    ...
    const shouldOpenControlUi = !opts.skipUi && settings.authMode === "token" && Boolean(settings.gatewayToken);
