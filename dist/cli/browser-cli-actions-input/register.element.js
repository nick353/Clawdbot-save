...
export function registerBrowserElementCommands(browser, parentOpts) {
    ...
    browser
        ...
        const { parent, baseUrl, profile } = resolveBrowserActionContext(cmd, parentOpts);
        try {
            const result = await browserAct(baseUrl, { kind: "press", key, targetId: opts.targetId?.trim() || undefined }, { profile });
            if (parent?.json) {
                defaultRuntime.log(JSON.stringify(result, null, 2));
                return;
            }
    ...
    browser
        ...
        const { parent, baseUrl, profile } = resolveBrowserActionContext(cmd, parentOpts);
        try {
            const result = await browserAct(baseUrl, { kind: "hover", ref, targetId: opts.targetId?.trim() || undefined }, { profile });
            if (parent?.json) {
                defaultRuntime.log(JSON.stringify(result, null, 2));
                return;
            }
