import { resolveAckReaction, resolveEffectiveMessagesConfig, resolveHumanDelayConfig, resolveIdentityName, } from "../../agents/identity.js";
...
import { buildPendingHistoryContextFromMap, clearHistoryEntries, } from "../../auto-reply/reply/history.js";
    ...
    if (removeAckAfterReply && ackReactionPromise && ackReaction) {
        ...
        void ackReactionPromise.then((didAck) => {
            ...
            removeReactionDiscord(message.channelId, message.id, ackReactionValue, {
                ...
                logVerbose(`discord: failed to remove ack reaction from ${message.channelId}/${message.id}: ${String(err)}`);
            });
        });
    }
    if (isGuildMessage && historyLimit > 0) {
