...
async function loadWebMediaInternal(mediaUrl, options = {}) {
    ...
    const optimizeAndClampImage = async (buffer, cap, meta) => {
        ...
        if (optimized.buffer.length > cap) {
            throw new Error(`Media could not be reduced below ${(cap / (1024 * 1024)).toFixed(0)}MB (got ${(optimized.buffer.length /
    ...
    const clampAndFinalize = async (params) => {
        ...
        if (params.kind === "image") {
            ...
            if (isGif || !optimizeImages) {
                if (params.buffer.length > cap) {
                    throw new Error(`${isGif ? "GIF" : "Media"} exceeds ${(cap / (1024 * 1024)).toFixed(0)}MB limit (got ${(params.buffer.length /
                        (1024 * 1024)).toFixed(2)}MB)`);
                }
                return {
        ...
        if (params.buffer.length > cap) {
            throw new Error(`Media exceeds ${(cap / (1024 * 1024)).toFixed(0)}MB limit (got ${(params.buffer.length /
                (1024 * 1024)).toFixed(2)}MB)`);
        }
        return {
            buffer: params.buffer,
