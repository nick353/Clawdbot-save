...
export async function sendMessageTelegram(to, text, opts = {}) {
    ...
    const timeoutSeconds = typeof account.config.timeoutSeconds === "number" &&
        ...
        : undefined;
    const client = fetchImpl || timeoutSeconds
        ? {
    ...
    if (mediaUrl) {
        ...
        let result;
        if (isGif) {
            result = await request(() => api.sendAnimation(chatId, file, mediaParams), "animation").catch((err) => {
                throw wrapChatNotFound(err);
            });
        }
        else if (kind === "image") {
            result = await request(() => api.sendPhoto(chatId, file, mediaParams), "photo").catch((err) => {
                throw wrapChatNotFound(err);
            });
        }
        else if (kind === "video") {
            result = await request(() => api.sendVideo(chatId, file, mediaParams), "video").catch((err) => {
                throw wrapChatNotFound(err);
            });
        }
        else if (kind === "audio") {
            ...
            if (useVoice) {
                result = await request(() => api.sendVoice(chatId, file, mediaParams), "voice").catch((err) => {
                    throw wrapChatNotFound(err);
                });
            }
            else {
                result = await request(() => api.sendAudio(chatId, file, mediaParams), "audio").catch((err) => {
                    throw wrapChatNotFound(err);
                });
            }
        }
        else {
            result = await request(() => api.sendDocument(chatId, file, mediaParams), "document").catch((err) => {
                throw wrapChatNotFound(err);
            });
        }
        ...
        if (needsSeparateText && followUpText) {
            const textParams = hasThreadParams || replyMarkup
                ...
                : undefined;
            const textRes = await request(() => textParams
                ? api.sendMessage(chatId, followUpText, textParams)
                : api.sendMessage(chatId, followUpText), "message").catch((err) => {
                throw wrapChatNotFound(err);
            });
    ...
    const res = await request(() => api.sendMessage(chatId, htmlText, textParams), "message").catch(async (err) => {
        ...
        if (PARSE_ERR_RE.test(errText)) {
            ...
            const plainParams = hasThreadParams || replyMarkup
                ? {
                    ...threadParams,
                    ...(replyMarkup ? { reply_markup: replyMarkup } : {}),
                }
            ...
            return await request(() => plainParams
                ? api.sendMessage(chatId, fallbackText, plainParams)
                : api.sendMessage(chatId, fallbackText), "message-plain").catch((err2) => {
