...
const PROFILE_DIR = '/root/clawd/browser-profiles/x';
...
async function main() {
  ...
  console.log('🐦 X (Twitter) ブラウザプロファイル初期化');
  console.log('='.repeat(50));
  console.log('');
  ...
  // X (Twitter)にアクセス
  console.log('');
  console.log('🌐 X (Twitter) にアクセスしています...');
  ...
  // スクリーンショット
  await page.screenshot({ path: '/tmp/x-profile-init.png', fullPage: false });
  console.log('📸 スクリーンショット: /tmp/x-profile-init.png');
  ...
  console.log('node post-to-x-v2-anti-ban.cjs "テキスト" [画像パス]');
  console.log('');
}
