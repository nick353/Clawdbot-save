...
## 🎯 プラットフォーム別最適化 (v6.1 - NEW! 2026-02-25)

**機能:**
- 各SNSプラットフォームの特性に合わせたキャプション生成
- Gemini Vision APIによる画像分析 + プラットフォーム戦略の統合
- エンゲージメント最大化のための最適化

### 📊 プラットフォーム別戦略

| プラットフォーム | コンテンツ戦略 | エンゲージメント最適化 |
|-----------------|----------------|------------------------|
| **Instagram** | 📹 リール（ショート動画）最優先（+25%） | ハッシュタグ5-10個、絵文字多め |
| **X (Twitter)** | 📹 動画が最高パフォーマンス | 質問ツイート、ユーモラス、2-3個ハッシュタグ |
| **Threads** | 🖼️ 画像と動画同等（画像わずかに上） | 会話重視、質問スレッド、1-2個ハッシュタグ |
| **Facebook** | 📹 ショート動画→画像の順 | ルール・オブ・サード、コミュニティ感 |
| **Pinterest** | 🖼️ 画像最適（ビジュアル検索） | 縦型ピン、キーワード最適化、5-10個ハッシュタグ |
...
# Instagram向けキャプション生成
python3 generate-caption-with-gemini.py /path/to/image.jpg --platform instagram

# X (Twitter)向けキャプション生成
python3 generate-caption-with-gemini.py /path/to/image.jpg --platform x

# Threads向けキャプション生成
python3 generate-caption-with-gemini.py /path/to/image.jpg --platform threads

# Facebook向けキャプション生成
python3 generate-caption-with-gemini.py /path/to/image.jpg --platform facebook

# Pinterest向けキャプション生成
python3 generate-caption-with-gemini.py /path/to/image.jpg --platform pinterest
```
...
【キャプション】
✨ 猫アート新作できました！ 🐱🎨
浮世絵スタイルの猫ちゃん、超かわいいです 💕
今日の気分にぴったり！
#猫アート #浮世絵 #JapaneseCat #UkiyoeArt #NekoChan

【ハッシュタグ】
#猫好きさんと繋がりたい #artwork #illustration #catlover #catstagram
```
...
将来的に `post-to-all-sns.sh` に統合予定:
1. 画像/動画を分析（Gemini Vision API）
2. 各プラットフォーム向けにキャプションを最適化
3. 一括投稿実行
