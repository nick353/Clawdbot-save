...
if (!postUrl) {
  console.error('❌ 使い方: node delete-facebook-post-v2.cjs <post_url>');
  process.exit(1);
}

async function deleteFacebookPost(postUrl, retryCount = 0) {
  console.log(`🗑️ Facebook投稿削除開始... (試行 ${retryCount + 1}/2)`);
  ...
  try {
    ...
    const beforePath = `${screenshotDir}/fb-${timestamp}-before.png`;
    ...
    const moreClicked = await page.evaluate(() => {
      ...
      for (const btn of btns) {
        ...
        if (ariaLabel.includes('More') || ariaLabel.includes('その他') || ariaLabel.includes('アクション')) {
          btn.click();
          return true;
        }
      }
    ...
    const deleteClicked = await page.evaluate(() => {
      ...
      for (const item of items) {
        ...
        if (txt.includes('Delete post') || txt.includes('投稿を削除') || txt.includes('削除')) {
          item.click();
          return txt;
        }
      }
    ...
    const afterPath = `${screenshotDir}/fb-${timestamp}-after.png`;
    ...
    if (retryCount === 0) {
      ...
      return deleteFacebookPost(postUrl, 1);
    } else {
      throw error;
    }
  } finally {
