#!/usr/bin/env node
/**
 * Instagram Reels 投稿スクリプト
 * Cookie JSONファイルから直接セッション復元 → 動画投稿
 *
 * Usage: node post-to-instagram-reels.cjs <video_path> <caption>
 */
...
if (!videoPath || !caption) {
  console.error('使い方: node post-to-instagram-reels.cjs <video_path> <caption>');
  process.exit(1);
}
