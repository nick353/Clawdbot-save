...
if (!imagePath || !caption) {
  console.error('使い方: node post-to-instagram-v6-improved.cjs <image_path> <caption>');
  process.exit(1);
}
...
async function shot(page, label) {
  ...
  const p = `/tmp/ig-imp-${label}-${ts}.png`;
...
async function main() {
  console.log('📸 Instagram v6-improved');
  ...
  try {
    ...
    for (let retry = 0; retry < 2; retry++) {
      try {
        ...
        // 投稿完了待機
        console.log('⏳ 投稿完了待機 (20秒)...');
        await randomDelay(18000, 22000);
